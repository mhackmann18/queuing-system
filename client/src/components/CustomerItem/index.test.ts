...
test('displays customer information correctly', () => {
  ...
  expect(screen.getByText(get12HourTimeString(mockCustomer.checkInTime))).toBeInTheDocument();
  expect(screen.getByText(get12HourTimeString(mockCustomer.callTime))).toBeInTheDocument();
});
...
test('applies styles based on customer status', () => {
  ...
  expect(screen.getByTestId('customer-item')).toHaveClass('border-slate_gray-700');
  expect(screen.getByTestId('customer-item')).toHaveClass('text-slate_gray-700');
